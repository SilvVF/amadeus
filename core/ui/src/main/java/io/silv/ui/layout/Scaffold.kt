...
fun Scaffold(
    ...
    floatingActionButtonPosition: io.silv.ui.layout.FabPosition = io.silv.ui.layout.FabPosition.End,
...
private fun ScaffoldLayout(
    fabPosition: io.silv.ui.layout.FabPosition,
    ...
    Layout(
        ...
        val fabPlacement = if (fabWidth > 0 && fabHeight > 0) {
            ...
            val fabLeftOffset = when (fabPosition) {
                io.silv.ui.layout.FabPosition.Start -> {
                ...
                io.silv.ui.layout.FabPosition.End, io.silv.ui.layout.FabPosition.EndOverlay -> {
                    if (layoutDirection == LayoutDirection.Ltr) {
                        layoutWidth - FabSpacing.roundToPx() - fabWidth
                    } else {
                        FabSpacing.roundToPx()
        ...
        val fabOffsetFromBottom = fabPlacement?.let {
            if (fabPosition == io.silv.ui.layout.FabPosition.EndOverlay) {
                it.height + FabSpacing.roundToPx() + bottomInsetsHeight
            } else {
                // Total height is the bottom bar height + the FAB height + the padding
                // between the FAB and bottom bar
                bottomBarHeight.coerceAtLeast(bottomInsetsHeight) + it.height + FabSpacing.roundToPx()
            }
        }
        val snackbarOffsetFromBottom = if (snackbarHeight != 0) {
            snackbarHeight + (fabOffsetFromBottom ?: 0).coerceAtLeast(
                bottomBarHeight.coerceAtLeast(
                    bottomInsetsHeight
                )
            )
        } else {
        ...
        topContentPadding = topBarHeight.coerceAtLeast(topInsetsHeight).toDp()
        bottomContentPadding = (fabOffsetFromBottom ?: 0).coerceAtLeast(
            bottomBarHeight.coerceAtLeast(bottomInsetsHeight)
        ).toDp()
        startContentPadding = startBarWidth.coerceAtLeast(startInsetsWidth).toDp()
