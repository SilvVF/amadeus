...
public suspend inline fun <T, reified V> ApiResponse<T>.mapSuccess(transformer: T.() -> V): ApiResponse<V> {
  if (this is ApiResponse.Success<T>) {
    return ApiResponse.of(data = transformer(this.data)) { this.raw }
  }
...
@JvmSynthetic
@Suppress("UNCHECKED_CAST")
public suspend inline fun <T, reified V> ApiResponse<T>.suspendMapSuccess(transformer: T.() -> V): ApiResponse<V> {
  if (this is ApiResponse.Success<T>) {
    val invoke = transformer.invoke(data)
    return ApiResponse.of(data = invoke) { this.raw }
  }
  return this as ApiResponse<V>
}
