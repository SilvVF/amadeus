...
class ExpandableState(
    ...
    internal val dragState: DraggableState =
                ...
                dragChannel.consumeAsFlow().collectLatest { action ->
                    when (action) {
                        ...
                        is DragAction.Stopped -> {
                            ...
                            val newProgress = progressFromHeight(height)
    ...
    suspend fun toggleProgress() {
        when (jobToTarget?.second ?: progressFromHeight(maxHeightPx - dragHeightOffset.targetValue)) {
            Hidden -> show()
            Expanded -> hide()
